...
int main(int argc, char **argv){
    ...
    ifstream readFile;
    readFile.open("/home/sunh/yammar_ws/src/controldis/data2.txt", ios::in);
    if (!readFile.is_open()){
        cout << "打开文件失败" << endl;
    }
    ...
    while (ros::ok())
    {
        ...
        if (flag ==0 ){
          alpha = theta_goal - pi * (90 - curr_pos.theta) / 180.0;
        }
        else{
         alpha = theta_goal - pi * (270 - curr_pos.theta) / 180.0;
        }
