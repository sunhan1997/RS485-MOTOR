      ...
      void RecePro(std::string s , double& lat , double& lon,  double& hrad_dir)
      {
          //分割有效数据，存入vector中
...
void gaussBLtoXY(double longitude,double latitude, double &X, double &Y)
 {
    ...
    int main (int argc, char** argv) 
    {
          ...
          ros::Publisher gps_headdir = nh.advertise<std_msgs::Float64>("gps_head_dir", 1000);
                  ...
                  while (i < strRece.length()) {
                      ...
                      //如果找到了起始标志，开始找终止标志
                          ...
                          //如果找到了终止标志，把这段有效的数据剪切给解析的函数，剩下的继续开始寻找
                              ...
                              double lat, lon, dir;
                              double x,y;
                              RecePro(strRece.substr(start, end + 2 - start), lat, lon, dir);
