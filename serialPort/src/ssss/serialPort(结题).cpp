...
//          ros::Publisher gps_first_x = nh.advertise<std_msgs::String>("gps_x", 1000);
//          ros::Publisher gps_first_y = nh.advertise<std_msgs::String>("gps_y", 1000);
          try {
              //设置串口属性，并打开串口
                  ...
                  while (i < strRece.length()) {
                      ...
                      if (start == -1) {
                          ...
                          if (end == -1 || end < start) {
                           ...
                           //  std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(9) << "纬度：" << lat
                               //        << " 经度：" << lon << "\n";
                              double x,y;
